...
export default function Dashboard() {
  ...
  const itensAtencao = equipamentos.filter(
  ...
  return (
    <div className="relative flex w-full flex-1 flex-col gap-8">
      <div className="flex flex-wrap items-start gap-4">
        <div>
          <p className="mb-1 text-[11px] font-extrabold tracking-[1.4px] text-brand-dark">
          ...
          <p className="mt-0.5 text-sm text-app-muted">
            Acompanhe equipamentos, estoque e atendimentos em um só lugar.
          </p>
        </div>
        <div className="flex gap-2 sm:absolute sm:right-0 sm:top-0">
          ...
          <Link href="/chamados" className="btn btn-primary">
            + Abrir chamado
          </Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(360px,.9fr)]">
          <div className="card min-w-0">
            <div className="card-header">
              ...
              <Link href="/chamados" className="btn btn-ghost btn-sm">
                Ver todos
              </Link>
            </div>
              ...
              <table>
                ...
                <tbody>
                  {chamados.slice(0, 5).map((chamado) => (
                    <tr key={chamado.id}>
                      <td className="font-semibold">{chamado.codigo}</td>
          ...
          <div className="card">
            <div className="card-header">
              <span className="card-title">Atenção no estoque</span>
              <span className="badge badge-atencao">
                {itensAtencao.length} itens
              </span>
            </div>
            <div className="card-body p-6">
              {itensAtencao.slice(0, 6).map((item) => (
                <div
                  className="flex items-center justify-between gap-5 border-b border-app-border py-4 last:border-b-0"
                  key={item.id}
                >
                  <div className="flex min-w-0 flex-col gap-0.5">
                    <strong>{item.nome}</strong>
                    <span className="text-xs text-app-muted">
                      {item.codigo} · limite de atenção: 3
                    </span>
                  </div>
                  <b className="whitespace-nowrap text-danger">
                    {item.quantidade} un.
                  </b>
                </div>
              ))}
              <Link
                href="/estoque/atencao"
                className="btn btn-primary mt-4 w-full justify-center"
              >
                Ver estoque em atenção
              </Link>
            </div>
          </div>
        </div>
...
function Metric({ valor, label, Icon, alerta = false }) {
  return (
    <div
      className={`relative flex min-h-30 items-center justify-center rounded-lg border border-app-border border-t-[3px] bg-app-surface p-6 shadow-sm ${alerta ? "border-t-warning" : "border-t-brand"}`}
    >
      <div className="flex flex-col items-center text-center">
        <span className="text-[28px] font-bold leading-none text-app-text">
          {valor}
        </span>
        <span className="mt-1 text-[13px] text-app-muted">{label}</span>
      </div>
      <span
        className={`absolute right-0.5 top-0.5 flex size-11 items-center justify-center rounded-xl ${alerta ? "bg-warning-light text-warning" : "bg-brand-soft text-brand-dark"}`}
        aria-hidden="true"
      >
