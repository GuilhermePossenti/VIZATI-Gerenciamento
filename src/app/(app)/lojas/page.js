...
export default function Lojas() {
  ...
  return <div className={styles.pagina}>
    ...
    <div className="card"><div className="tabela-wrapper"><table><thead><tr><th>Código</th><th>Loja</th><th className={styles.colunaAcoes}>Ações</th></tr></thead><tbody>{lojas.map((loja) => <tr key={loja.id}><td><b>{loja.codigo}</b></td><td>{loja.nome}</td><td className={styles.colunaAcoes}><div className={styles.ajustes}><button className="btn btn-secondary btn-sm" onClick={() => { setEditando(loja); setForm({ codigo: loja.codigo, nome: loja.nome }); setAberto(true); }}>Alterar</button><button className="btn btn-danger btn-sm" onClick={() => excluir(loja)}>Excluir</button></div></td></tr>)}</tbody></table></div></div>
