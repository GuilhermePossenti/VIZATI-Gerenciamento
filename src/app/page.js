...
export default function LoginPage() {
  ...
  const entrar = async (e) => {
    ...
    if (!loginInput.trim() || !senha)
      return setErro("Informe seu usuário e senha.");
    setCarregando(true);
    setErro("");
    await new Promise((r) => setTimeout(r, 350));
    const resultado = login(loginInput, senha);
    setCarregando(false);
    if (resultado.ok) router.push("/dashboard");
    ...
    <main className={styles.pagina}>
      ...
      <section className={styles.card}>
        <div className={styles.marca}>
          <img src={`${appBasePath}/logo.png`} alt="SUPERVIZA TI" />
          <div>
            <strong>SUPERVIZA TI</strong>
        ...
        <div className={styles.conteudo}>
          ...
          <p className={styles.subtitulo}>
            Entre para gerenciar o estoque e os atendimentos da sua operação de
            TI.
          </p>
          ...
          <div className={styles.demo}>
            ...
            <div className={styles.credenciaisDemo}>
              {credenciaisDemo.map(([usuario, senha]) => (
                <code key={usuario}>
                  {usuario} / {senha}
                </code>
              ))}
